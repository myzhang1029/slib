...
OPT double S_CDECL calculator( ccp infix ) //整合功能，提供计算
{
	double result;
...
*/
OPT int S_CDECL InfixToPostfix(ccp infix,char *postfix)
{
	int j=0,i=0;
	char c,e;
	sqStack s;
...
*/
OPT int S_CDECL Calculate(char *arr,void *result)
{
...
*/
OPT int S_CDECL InitStack(sqStack *s,unsigned stackSize,unsigned typeSize)
{
...
压栈
*/
OPT int S_CDECL Push(sqStack *s,void *e)
{
...
出栈
*/
OPT int S_CDECL Pop(sqStack *s,void *e)
{
	if(s->top==s->base)
{
serr=3;
...
清栈
*/
OPT int S_CDECL ClearStack(sqStack *s)
{
	s->top=s->base;
	return STRUE;
}
...
毁栈
*/
OPT int S_CDECL DestroyStack(sqStack *s)
{
	free(s->base);
	s->top=s->base=NULL;
...
获取已入栈元素个数
*/
OPT int S_CDECL GetLen(sqStack *s)
{
...
自定义错误处理函数
*/
OPT int S_CDECL prterr(ccp fmt,... )
{
	vlist argptr;
	vstart(argptr, fmt);
...
OPT int S_CDECL mycmd( ccp path )
{
	char order[20];
	fflush( stdin );
	printf( "%s(Enter e to go back)>", path );
	fgets( order, 19, stdin );
	if ( strcmp(order,"e\n")==0 )
	{
		return STRUE;
	}
...
OPT long S_CDECL fsize( FILE * stream )
{
	int size;
	if ( !stream )
{
...
OPT int S_CDECL fcopy( ccp oldname, ccp newname ) /* user - sec7 */
{
	FILE * fold, *fnew;
...
OPT int S_CDECL prtpn( FILE*fp, cui minimum, cui maximum )
{
	int n=0,i=0,torf=0;
	if ( maximum<=2)
	{
...
OPT int S_CDECL ispn( cui testingnum )
{
	int i;
	if( testingnum == 2)
		return STRUE;
...
OPT int S_CDECL random( cui seed,cui maximum,cui minimum )
{
	srand(seed);
...
OPT char * S_CDECL myitoa(int num, char *str, int radix)
{
...
OPT int S_CDECL isrp (cui n1,cui n2)
{
	int counter;
	if(n1 == n2)
		return SFALSE;
...
OPT char * S_CDECL _strstr(ccp str1,ccp str2)
{
...
OPT char * S_CDECL mtscat(cui amount,...)
{
	vlist strs;
	char *str,*buf;
	int counter;
...
OPT int S_CDECL iofile(FILE *origin,FILE *dest)
{
	int ch;
	rewind(origin);
	if(feof(origin))
...
OPT int S_CDECL prtfile(FILE *stream)
{
	return iofile(stream,stdout);
}


OPT int S_CDECL average(cui amt,...)
{
	vlist args;
	int count,sum=0;
	vstart(args,amt);
...
OPT int S_CDECL gcf(cui n1,cui n2)
{
	int count;
	if(n1==n2)
		return n1;
...
OPT int S_CDECL lcm(cui n1,cui n2)
{
	return n1*n2/gcf(n1,n2);
}


OPT uslong S_CDECL wsbl_frame(int ch)
{
	switch ( tolwr(ch) )
	{
	case ('\n'):
...
OPT int S_CDECL rsbl_frame(int code)
{
	switch ( code )
	{
	case ('\n'):
...
OPT S_INLINE int S_CDECL initmem(void *mem)
{
	memset(mem,0,sizeof(mem));
	return 0;
}

OPT S_INLINE int S_CDECL tolwr(int chr)
{
...
OPT S_INLINE int S_CDECL toupr(int chr)
{
...
OPT S_INLINE int S_CDECL isalp(int chr)
{
...
OPT S_INLINE int S_CDECL isaln(int chr)
{
...
OPT S_INLINE int S_CDECL isdgt(int chr)
{
...
OPT S_INLINE int S_CDECL isupr(int chr)
{
	return chr>=65&&chr<=90?
	       STRUE:SFALSE;
}
...
OPT S_INLINE int S_CDECL islwr(int chr)
{
...
OPT S_INLINE int S_CDECL isctl(int chr)
{
...
OPT S_INLINE int S_CDECL isprt(int chr)
{
...
OPT S_INLINE int S_CDECL isgrh(int chr)
{
...
OPT S_INLINE int S_CDECL ispct(int chr)
{
...
OPT S_INLINE int S_CDECL isspc(int chr)
{
...
OPT S_INLINE int S_CDECL isxdgt(int chr)
{
...
OPT S_INLINE int S_CDECL gtc (FILE* __F)
{
	return (--__F->_cnt >= 0)?
...
OPT S_INLINE int S_CDECL ptc (int __c, FILE* __F)
{
	return (--__F->_cnt >= 0)?
...
OPT S_INLINE int S_CDECL gtch (void)
{
	return (--stdin->_cnt >= 0)?
...
OPT S_INLINE int S_CDECL ptch(int __c)
{
...
OPT S_INLINE int * S_CDECL _serrloc( void )
{
	return &serr;
}
