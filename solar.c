...
void slib_h2hms(double in_hours, double *out_hours, double *out_minutes, double *out_seconds)
{
    double hours_fracpart = modf(in_hours, out_hours); /* the decimal part is out_hours */
    hours_fracpart *= 60;
    *out_seconds = modf(hours_fracpart, out_minutes);
    *out_seconds *= 60;
}
/* convert time in hours, minutes and seconds to hours, stable API */
double slib_hms2h(double hours, double minutes, double seconds) { return hours + minutes / 60 + seconds / 3600; }
...
double slib_hour_angle_to_hour_diff(double hour_angle) { return hour_angle / 15.0; }
...
{
    return -dasin(0.39779 * dcos(0.98565 * (d + 10) + 1.914 * dsin(0.98565 * (d - 2))));
}
...
/* the sunrise equation with corrections
   ...
   cos(ωo) = (sin(0.83°-2.076°*sqrt(elevation)/60°) - sin(φ)*sin(δ))/(cos(φ)*cos(δ)) */
double slib_corr_sunrise_hour_angle(double latitude, double sun_declination, double elevation)
{
    return dacos((dsin(0.83 - 2.076 * sqrt(elevation) / 60) - dsin(latitude) * dsin(sun_declination)) /
...
int main()
{
    ...
    printf("%lf\n", slib_hour_angle_to_hour_diff(
                        slib_corr_sunrise_hour_angle(25.057672, slib_sun_decl_by_date(slib_d2dn(2018, 8, 30)), 1904)));
    return 0;
}
