...
/*
 * Modified for slib by Zhang Maiyun on May, 2017
 */
	 ...
	 const struct option *long_options, int *opt_index)
{
	...
	int count,optcount;
	options[0]=':';
	for(count=0,optcount=1;
	long_options[count].name!=NULL;
	++count,++optcount)
	{
		options[optcount]=long_options[count].val;
		if(long_options[count].has_arg==1||long_options[count].has_arg==2)
		{
			options[++optcount]=':';
			if(long_options[count].has_arg==2)
				options[++optcount]=':';
		}
	}
	return _getopt_internal(argc, argv, options, long_options, opt_index, 0);
